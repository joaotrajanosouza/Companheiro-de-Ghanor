...
const DiceFace: React.FC<DiceFaceProps> = ({ value }) => {
  ...
  return (
    <div className="w-12 h-12 bg-white dark:bg-[#e8dbc5] rounded-xl shadow-[inset_0_-2px_4px_rgba(0,0,0,0.2),0_2px_4px_rgba(0,0,0,0.5)] border border-gray-200 dark:border-[#d4c5ab] flex relative overflow-hidden">
      ...
      <div className="w-full h-full relative">
        {pips.map((i) => {
          ...
          return (
            <div 
              key={i} 
              className={`absolute w-2.5 h-2.5 rounded-full bg-slate-900 shadow-[inset_0_1px_2px_rgba(0,0,0,0.8)] ${posClass}`} 
            />
          );
        })}
      </div>
